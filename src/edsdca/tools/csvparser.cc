...
using tools = edsdca::tools;

std::pair<Eigen::VectorXd, Eigen::MatrixXd> tools::CsvParser::Parse(const std::string& data,
  ...
  labels = Eigen::VectorXd(num_data);
  features = Eigen::MatrixXd::Zeros(num_features, num_data);
  ...
  for (const auto& line : lines) {
    ...
    for (long i = 0; i < split_row.size(); ++i) {
      auto val = tools::String::cast_to_type<double>(split_row[i]);
      if (i == label_pos) {
        labels << val;
      } else {
